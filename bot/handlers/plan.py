...
async def process_start_date(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("Не понял дату. Напиши в формате ГГГГ-ММ-ДД, например 2026-02-11.")
        return
...
async def process_first_time(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer("Не понял время. Напиши в формате ЧЧ:ММ, например 10:00.")
        return
...
async def process_goal(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "Сколько постов в день планируем?", reply_markup=_posts_per_day_keyboard()
    )
    await state.set_state(PlanStates.waiting_for_posts_per_day)


@router.callback_query(
    PlanStates.waiting_for_posts_per_day, F.data.startswith("ppd_")
)
...
async def _generate_and_show_plan(message: types.Message, state: FSMContext):
    ...
    # 1. Форматируем и отправляем план пользователю
    ...
    # 2. Переводим FSM в состояние ожидания решения по плану
    await state.set_state(PlanStates.waiting_for_plan_decision)



@router.callback_query(
    PlanStates.waiting_for_plan_decision, F.data == "plan_regenerate"
)
...
@router.callback_query(
    PlanStates.waiting_for_plan_decision, F.data == "plan_cancel"
)
async def plan_cancel(callback: types.CallbackQuery, state: FSMContext):
    await callback.message.edit_reply_markup(reply_markup=None)
    await callback.message.answer("План на эту неделю отменён, ничего не сохранил.")
    await state.clear()
    await callback.answer()


@router.callback_query(
    PlanStates.waiting_for_plan_decision, F.data == "plan_accept"
)
async def plan_accept(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not plan:
        await callback.message.answer("Не нашёл текущий план. Попробуй ещё раз через /plan_week.")
        await state.clear()
        await callback.answer()
        return
    ...
    await callback.message.answer(
        f"Готово! План сохранён (id: <code>{resp['plan_id']}</code>, "
        f"количество постов: {resp['items_count']}).\n\n"
        "Теперь можно смотреть его через /plan_status (сделаем чуть позже) "
        "и в будущем настроить автопостинг."
    )
    await state.clear()
    await callback.answer()
